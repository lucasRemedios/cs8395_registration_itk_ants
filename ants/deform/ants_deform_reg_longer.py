...
for img in range(len(files)):
    ...
    deformable_reg = ants.registration(fixed, moving, type_of_transform="ElasticSyN", reg_iterations=(20, 10, 5), verbose=1)

    transformed_img = ants.apply_transforms(fixed, moving, transformlist=deformable_reg["fwdtransforms"])


    transformed_img.to_filename("20_10_5_deform_"+files[img])
